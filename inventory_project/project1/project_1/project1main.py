...
class Web_Data:
    ...
    def web_data(self, *args):
        action = args[0]

        if action == "ADD":
            # Add item to the inventory
            return_str = inventory.add_method(*args[1:])

        elif action == "REMOVE":
            return_str = inventory.update_method(*args[1:3])

        elif action == "BRING":
            excel_df = inventory.bring_list()

        elif action == "SEARCH":
            result = inventory.search_method(*args[1])

        elif action == 'PRINT':
            return_str = inventory.print_list(*args[1])

        elif action == 'BULK':
            return_str = inventory.bulk_entry(*args[1])
